...
def main() -> None:
    ...
    parser.add_argument("-c", "--config", default="cassandra_jmx_mon.yaml", help="YAML configuration file.")
            ...
            while not _stop:
                ...
                for oname, attributes in _makeNames(conn, config):
                    ...
                    try:
                        ...
                        logging.error(
                            "Failed to get attributes oname=%s attributes=%s: %s", oname, attributes, exc
                        )
                        continue
                    except:  # noqa
